//Program generating a whole binary tree at run time. Supporting Level order traversal using naive solution of complexity >O(n). It uses height() function to get height of tree and then call klevel() function to print kth level for every level till equal to height calls made
...
void node::levelOrder(node *root){  //function to print level order traversal of any binary tree
    int n=height(root); //storing height of tree
    if(n==0){  //if height is zero
        cout<<"Tree's Empty!"<<endl;
        return;
    }
    for(int i=0;i<n;++i)
        klevel(root,i);  //calling klevel() to print nodes at every level in height of binary tree
}
...
int main(){
    ...
    cout<<"9. Print level-order traversal"<<endl;
    int n;
    while(cin>>n){
        switch(n){
        case 1:
